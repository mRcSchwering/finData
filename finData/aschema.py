...
class ASchema(object):
    ...
    def table(self, name):
        ...
        return Table(name, update_rate, cols, col_types)
...
class Table(object):
    ...
    def __init__(self, table_name, update_rate, columns, column_types):
        self.name = table_name
        self.update_rate = update_rate
        self.columns = columns
        self._column_types = column_types
        self.insert_statement = self._prepareInsertStatement()
    ...
    def _prepareInsertStatement(self):
        cols = ','.join(self.columns)
        vals = ','.join(['%({v})s'.format(v=v) for v in self.columns])
